...
describe('stageOrder', () => {
  it('walks the pipeline forward from the entry', () => {
    expect(stageOrder(graph)).toEqual(['intake', 'plan', 'build', 'qa', 'accept', 'review', 'merge', 'merged']);
  });

  it('terminates on a cycle rather than hanging the browser', () => {
    const g = defaultGraph();
    g.edges.push(['merged', 'intake']);          // a malformed graph, saved by hand
    expect(() => stageOrder(g)).not.toThrow();
    expect(stageOrder(g)).toHaveLength(8);
  });

  it('stops where the chain breaks', () => {
    const g = defaultGraph();
    g.edges = g.edges.filter(e => e[0] !== 'build');
...
describe('runSnapshotForTask', () => {
  it('marks earlier stages done, the current one running, later ones pending', () => {
    const r = runSnapshotForTask(graph, task({ stage: 'qa', status: 'WORKING' }));
  ...
  it('a DONE task shows every stage succeeded, whatever `stage` still says', () => {
    ...
    const r = runSnapshotForTask(graph, task({ stage: 'merge', status: 'DONE' }));
  ...
  it('BLOCKED marks the current stage failed, not running — nothing is running', () => {
    const r = runSnapshotForTask(graph, task({ stage: 'build', status: 'BLOCKED', lastError: 'attempts exhausted' }));
  ...
  it('a failed QA verdict renders as rejected', () => {
    const r = runSnapshotForTask(graph, task({ stage: 'qa', status: 'WORKING', qaVerdict: 'fail' }));
    expect(r.stages.qa.state).toBe('rejected');
  });

  it('a task parked at the human gate is not "running"', () => {
    const r = runSnapshotForTask(graph, task({ stage: 'review', status: 'TESTING' }));
  ...
  it('a task with no stage has nothing done and nothing running', () => {
    const r = runSnapshotForTask(graph, task({ stage: null, status: 'TODO' }));
  ...
  it('a stage the graph does not contain does not fabricate progress', () => {
    ...
    const r = runSnapshotForTask(graph, task({ stage: 'blocked', status: 'WORKING' }));
    expect(Object.values(r.stages).every(s => s.state === 'pending')).toBe(true);
  });

  it('carries the task log path through, so the popup can link to it', () => {
    const r = runSnapshotForTask(graph, task({ stage: 'qa', logPath: 'C:/logs/default/T-1.log' }));
...
describe('edgeState', () => {
  const run = runSnapshotForTask(graph, task({ stage: 'qa', status: 'WORKING' }));
  ...
  it('an edge into a failed stage counts as traversed — work did flow there', () => {
    const blocked = runSnapshotForTask(graph, task({ stage: 'build', status: 'BLOCKED' }));
