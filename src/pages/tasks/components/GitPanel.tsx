...
// Colorized unified-diff block (dark), reused by the changes viewer and history drill-down.
function DiffView({ diff }: { diff: string }) {
  return (
    <pre className="text-[11px] leading-relaxed font-mono p-3 overflow-x-auto custom-scrollbar max-h-[40vh] overflow-y-auto">
      {(diff ?? '').split('\n').map((line, i) => {
...
function SearchSelect({ value, onChange, options, placeholder, onOpen, loading, allowCustom, disabled, mono }: {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute z-30 mt-1 w-full max-h-56 overflow-auto custom-scrollbar bg-white border border-slate-200 rounded-xl shadow-lg py-1">
...
export function GitPanel({ isOpen, onClose, activeId }: GitPanelProps) {
  ...
  const deleteRepo = () => {
    ...
    (async () => {
      const ok = await confirm({ title: 'Delete this repo?', message: `Permanently delete the folder for “${dir}” from disk (and its project)? This removes the cloned repository.`, confirmLabel: 'Delete', tone: 'danger' });
      if (!ok) return;
  ...
  const tokenOptions = (val: string, onChange: (v: string) => void, filter?: (t: TokenMasked) => boolean) => (
    <select value={val} onChange={e => onChange(e.target.value)} className={`${inputCls} appearance-none`}>
  ...
  return (
    <div className="fixed inset-0 z-[1100] flex items-center justify-center p-2 sm:p-4 bg-black/80 backdrop-blur-md" onClick={onClose}>
      <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }}
        ...
        className="relative w-full max-w-2xl bg-white border border-slate-200 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[94vh]">
        {/* Header */}
        ...
        <div className="flex gap-1 px-2 sm:px-3 pt-2 overflow-x-auto custom-scrollbar border-b border-slate-100 shrink-0">
          {TABS.map(t => {
            const Icon = t.icon;
            return (
        ...
        <div className="p-4 sm:p-6 overflow-y-auto custom-scrollbar space-y-5 flex-1">
            ...
            <>
              <div className="flex flex-col sm:flex-row gap-2">
                <input value={repoPath} onChange={e => setRepoPath(e.target.value)} onKeyDown={e => e.key === 'Enter' && loadStatus()}
                  placeholder="repo/worktree path — blank = server cwd" autoComplete="off" className={`${inputCls} font-mono text-xs sm:text-sm`} />
                <button onClick={() => loadStatus()} disabled={statusLoading} className={`${btnGhost} shrink-0`}>
                          ...
                          <div key={f.path}>
                            ...
                            {active && (
                              <div className="mt-1.5 mb-2 rounded-lg border border-slate-200 bg-slate-900 overflow-hidden">
                                {diffLoading ? <div className="px-3 py-4 text-xs text-slate-400">Loading diff…</div> : <DiffView diff={diff ?? ''} />}
                              </div>
                            )}
                          </div>
                        );
                  ...
                  <div className="pt-2 border-t border-slate-100 space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-col sm:flex-row gap-2">
                        ...
                        <button onClick={doBranch} disabled={acting} className={`${btnGhost} shrink-0`}>Create</button>
                      </div>
                    </div>
                    {/* pull + push */}
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-col sm:flex-row gap-2">
                        ...
                        <button onClick={doPush} disabled={acting} className={`${btnPrimary} shrink-0`}><Upload size={14} /> Push</button>
                      </div>
                    </div>
                    {msgBox(actMsg)}
                  </div>
          ...
          {tab === 'clone' && (
            <div className="space-y-3">
              <p className="text-[11px] text-slate-500 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2">Pick a token first — for a GitHub App it lists the repos you can clone. Leave token blank only for public repos (then paste the URL).</p>
              {/* 1) token — everything below depends on it */}
              <div><label className="block text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-1">Token</label>
                {tokens.length === 0 ? (
                ...
                ) : tokenOptions(cloneToken, setCloneToken)}</div>
              {/* 2) repo picker — populated from the GitHub App installation's accessible repos */}
              {cloneToken.startsWith('app:') && (
                <div>
                  <label className="block text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-1">
                    Repository {repoListBusy && <span className="text-slate-400 normal-case font-normal">— loading…</span>}
                  </label>
                  {repoListErr ? (
                    <div className="text-[11px] text-rose-600 bg-rose-50 border border-rose-200 rounded-lg px-2.5 py-1.5">{repoListErr}</div>
                  ) : repoList.length === 0 && !repoListBusy ? (
                    <div className="text-[11px] text-slate-400 bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1.5">No repos found for this app — install it on the repos you want.</div>
                  ) : (
                    <SearchSelect
                      value={cloneUrl}
              ...
              <div><label className="block text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-1">Repository URL</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-1">Target directory</label>
                ...
                <div><label className="block text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-1">Branch <span className="text-slate-400 normal-case font-normal tracking-normal">{rbBusy ? '— loading…' : '(searchable)'}</span></label>
                  <SearchSelect
                    value={cloneBranch}
              ...
              {indexing && <p className="text-[11px] text-slate-500">Indexing the repo so agents can search it… (automatic)</p>}
            </div>
          )}
          ...
          {tab === 'run' && (
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                <button onClick={detectRun} disabled={detecting} className={`${btnGhost} flex-1`}>
                ...
                <button onClick={saveRunConfig} disabled={savingRun} className={`${btnGhost} shrink-0`}><Save size={15} /> Save</button>
              </div>
              {detectSource && <div className="text-[10px] text-slate-400">Detected stack: <span className="font-mono">{detectSource}</span></div>}
              ...
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-1">Subdirectory (optional)</label>
                ...
                <div className="rounded-xl border border-slate-200 overflow-hidden">
                  <div className="px-3 py-2 bg-slate-900 flex items-center gap-2">
                    <span className="text-[11px] font-bold text-white flex items-center gap-1.5">
                      {runRunning ? <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" /> : <span className={`w-2 h-2 rounded-full ${runExit === 0 ? 'bg-emerald-400' : 'bg-rose-400'}`} />}
          ...
          {tab === 'tokens' && (
            <div className="space-y-5">
              ...
              <div>
                <label className="block text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-1.5">Where's your code?</label>
                    ...
                    {patTokens.map(t => (
                      <div key={t.id} className="flex items-center gap-2 p-2.5 rounded-xl border border-slate-200 bg-slate-50">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <div className="text-[11px] text-slate-500 font-mono break-all">{t.tokenMasked} · {t.host}</div>
                        </div>
                        ...
                        <button
                          ...
                          className="p-2 min-h-[40px] min-w-[40px] flex items-center justify-center text-slate-400 hover:text-rose-600 rounded-lg"
                      ...
                      return (
                        <div key={app.id} className="p-2.5 rounded-xl border border-slate-200 bg-slate-50 space-y-2">
                          ...
                          {app.state === 'installed' ? (
                            ...
                            <div className="flex flex-col sm:flex-row gap-2">
                              {installUrl && (
                                <a href={installUrl} target="_blank" rel="noopener noreferrer" className={`${btnGhost} text-xs px-3 shrink-0`}><ExternalLink size={13} /> Install on GitHub</a>
                              )}
                              <button onClick={() => detectInstallation(app.id)} disabled={detectingId === app.id} className={`${btnGhost} text-xs px-3 shrink-0`}>
              ...
              <div className="rounded-xl border border-slate-200 overflow-hidden">
                ...
                <div className="p-3 space-y-3 border-t border-slate-100">
                  ...
                  <div className="text-[11px] text-amber-800 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 flex items-start gap-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <div>
                      <label className="block text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-1">App name (optional)</label>
                      <input value={appName} onChange={e => setAppName(e.target.value)} placeholder="ai-agents-git" className={inputCls} />
                    </div>
                    <div>
                      <label className="block text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-1">Organization (blank = personal)</label>
                      <input value={appOrg} onChange={e => setAppOrg(e.target.value)} placeholder="my-org" className={inputCls} />
                    </div>
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-1">Access</label>
                  ...
                  <div className="text-[11px] text-slate-500 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2">
                    ...
                    <div className="mt-1.5 flex flex-wrap gap-1.5">
                      {(appScope === 'readonly'
                        ...
                        <span key={p} className="text-[10px] font-bold rounded px-1.5 py-0.5 bg-white border border-slate-200 text-slate-600">{p}</span>
                      ))}
                    </div>
                    {appScope === 'readonly' && (
                      <p className="mt-1.5 text-[10px] text-slate-400">Read-only can clone/pull but not push.</p>
                    )}
                  </div>
                  <button onClick={connectGithubApp} disabled={appBusy} className={`${btnPrimary} w-full`}>
                  ...
                  <div className="border-t border-slate-100 pt-2">
                    <button type="button" onClick={() => setManualOpen(o => !o)} className="w-full flex items-center justify-between gap-2 text-[11px] font-bold text-slate-500 hover:text-slate-700">
                    ...
                    {manualOpen && (
                      <div className="mt-2 space-y-2">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          <input value={manualAppId} onChange={e => setManualAppId(e.target.value)} inputMode="numeric" placeholder="App ID e.g. 123456" className={inputCls} />
                          <input value={manualName} onChange={e => setManualName(e.target.value)} placeholder="name (optional)" className={inputCls} />
                        </div>
                        <textarea value={manualKey} onChange={e => setManualKey(e.target.value)} rows={4}
                          placeholder={'-----BEGIN RSA PRIVATE KEY-----\n...\n-----END RSA PRIVATE KEY-----'}
                          className={`${inputCls} font-mono text-[11px] resize-y`} autoComplete="off" spellCheck={false} />
                        <button onClick={connectManualApp} disabled={manualBusy || !manualAppId.trim() || !manualKey.trim()} className={`${btnGhost} w-full`}>
                        ...
                        <p className="text-[10px] text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-2.5 py-1.5">The private key is a secret — stored locally, same as a PAT. Never shared.</p>
                      </div>
                    )}
                  </div>
              ...
              <div className="rounded-xl border border-slate-200 overflow-hidden">
                ...
                <div className="p-3 space-y-3 border-t border-slate-100">
                  {editingId && <div className="flex justify-end"><button onClick={resetTokenForm} className="text-[11px] font-bold text-slate-500 hover:text-slate-800">+ new instead</button></div>}
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  <input value={tLabel} onChange={e => setTLabel(e.target.value)} placeholder="label e.g. agents-readonly" className={inputCls} />
                  <select value={tScope} onChange={e => setTScope(e.target.value as any)} className={`${inputCls} appearance-none`}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  ...
                  <input value={tHost} onChange={e => setTHost(e.target.value)} placeholder={activeProvider.host} className={inputCls} />
                </div>
                {msgBox(tokMsg)}
                <div className="flex justify-end"><button onClick={saveToken} className={btnPrimary}><Plus size={15} /> {editingId ? 'Save' : 'Add token'}</button></div>
                </div>
                )}
              </div>
          ...
          {tab === 'agents' && (
            <div className="space-y-3">
              ...
              {agents.map(a => (
                <div key={a.role} className="flex items-center gap-2 p-2.5 rounded-xl border border-slate-200">
                  <Bot size={16} className="text-slate-400 shrink-0" />
                  <div className="flex-1 min-w-0"><div className="text-sm font-bold text-slate-800 truncate">{a.label || a.role}</div><div className="text-[10px] text-slate-400 uppercase tracking-wider">{a.role}</div></div>
                  <div className="w-[46%] max-w-[220px]">{tokenOptions(assignments[a.role] || '', v => setAssign(a.role, v))}</div>
                </div>
              ))}
              {agents.length === 0 && <p className="text-xs text-slate-400">No agents found.</p>}
              {msgBox(asgMsg)}
            </div>
          )}
          ...
          {tab === 'worktrees' && (
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-[11px] text-slate-500">Isolated worktrees where agents build each task.</span>
                <button onClick={loadWorktrees} disabled={wtLoading} className={btnGhost}><RefreshCw size={14} className={wtLoading ? 'animate-spin' : ''} /> Refresh</button>
              </div>
              {worktrees.length === 0 && !wtLoading && <p className="text-xs text-slate-400">No active agent worktrees.</p>}
              {worktrees.map(wt => (
                <div key={wt.path} className="p-3 rounded-xl border border-slate-200 space-y-2">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-[10px] font-black uppercase tracking-wider rounded px-1.5 py-0.5 bg-slate-100 border border-slate-200 text-slate-600">{wt.isPlan ? 'PLAN' : 'DEV'}</span>
                  ...
                  {wt.lastCommit?.subject && (
                    <div className="text-[11px] text-slate-500 bg-slate-50 border border-slate-100 rounded-lg px-2 py-1.5">
                      <span className="font-mono text-slate-700">{wt.lastCommit.sha}</span> {wt.lastCommit.subject}
                      <span className="text-slate-400"> — {wt.lastCommit.author}</span>
                    </div>
                  )}
                  <div className="flex gap-2">
                    <button onClick={() => openWorktree(wt)} className={`${btnGhost} text-xs px-3`}><FileDiff size={13} /> Changes</button>
                    <button onClick={() => historyForWorktree(wt)} className={`${btnGhost} text-xs px-3`}><History size={13} /> History</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          {tab === 'history' && (
            <div className="space-y-3">
              <div className="flex flex-col sm:flex-row gap-2">
                <input value={repoPath} onChange={e => setRepoPath(e.target.value)} placeholder="repo/worktree path — blank = cwd" className={`${inputCls} font-mono text-xs`} />
                <input value={histRef} onChange={e => setHistRef(e.target.value)} onKeyDown={e => e.key === 'Enter' && loadLog()} placeholder="branch/ref (optional)" className={`${inputCls} font-mono text-xs sm:max-w-[38%]`} />
                <button onClick={() => loadLog()} disabled={histLoading} className={`${btnGhost} shrink-0`}><RefreshCw size={14} className={histLoading ? 'animate-spin' : ''} /> Log</button>
              </div>
              {histErr && <div className="text-xs bg-rose-50 border border-rose-200 rounded-lg px-3 py-2 text-rose-700">{histErr}</div>}
              {commits.length === 0 && !histErr && !histLoading && <p className="text-xs text-slate-400">Load a repo to see who committed what.</p>}
              <div className="space-y-1.5">
                {commits.map(c => {
                    ...
                    <div key={c.hash}>
                      ...
                      {open && (
                        <div className="mt-1.5 mb-2 rounded-lg border border-slate-200 overflow-hidden">
                          {!showData ? <div className="px-3 py-3 text-xs text-slate-400">Loading…</div>
                            : showData.ok === false ? <div className="px-3 py-3 text-xs text-rose-600">Could not load commit.</div>
                            : (<>
                                <div className="px-3 py-2 bg-slate-50 border-b border-slate-100 text-[11px] text-slate-600">
                                  ...
                                  <div className="mt-1 flex flex-wrap gap-1">
                                    {(showData.files || []).map((f: any, i: number) => (
                                      <span key={i} className="font-mono text-[10px] rounded px-1.5 py-0.5 bg-white border border-slate-200 text-slate-600">{f.status} {f.path}</span>
                                    ))}
                                  </div>
                                </div>
          ...
          {tab === 'index' && (
            <div className="space-y-4">
              ...
              {idx && (
                <div className="rounded-xl border border-slate-200 overflow-hidden">
                  ...
                  <div className="p-3 space-y-2">
                    <div className="font-mono text-[11px] text-slate-700 break-all">{idx.root}</div>
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                <button onClick={loadIndex} className={btnGhost}><RefreshCw size={14} /> Refresh</button>
                <button onClick={rebuildIndex} disabled={idxBusy} className={btnPrimary}><HeartPulse size={15} /> Rebuild / Heal now</button>
              </div>

              <div className="p-3 rounded-xl border border-slate-200 space-y-2">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Index a different repo</label>
                ...
                <div className="flex flex-col sm:flex-row gap-2 sm:justify-end">
                  ...
                  <button onClick={() => retargetIndex(idxRoot.trim())} disabled={idxBusy} className={btnPrimary}><Database size={15} /> Index this repo</button>
                </div>
              </div>
